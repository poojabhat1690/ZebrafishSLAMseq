### plotting fraction uridylation of different classes of MZ genes
library(ggplot2)
library(dplyr)
...
masterTable_data_fractionU = masterTable_data %>% dplyr::select(dplyr::contains("fractionU"))
...
times = c(0,1,2,4,6,8)
pdf("/Volumes/Macintosh HD/Users/pooja.bhat/Dropbox/Paperoutline/figureOutline/paper_ZebrafishSLAMseq/figures/figure5//plots/FractionUridylation_ClustersMZgenes.pdf", height = 4, width=4)
...
for(i in 1:length(timepoints)){
  ...
  p =  ggplot(b_split_2hpf, aes((fractionU), color=V11,group=V11)) +
...
b = masterTable_data_fractionU  %>% dplyr::mutate(fractionU = X6hpf_fractionU/X2hpf_fractionU) %>% dplyr::filter(class=="M" | class=="MZ" | class == "M-stable") %>% dplyr::select(c('class','V11','fractionU','external_gene_name'))
...
p = ggplot(b_split_2hpf,aes(y=log10(fractionU),x=category_sample)) + geom_boxplot(outlier.shape = NA) + ggpubr::stat_compare_means(ref.group = 'M(n=297)') + theme_cowplot()
...
dev.off()
utails = do.call(rbind.data.frame,masterTable_data_fractionU)
write.table(utails,"/Volumes/Macintosh HD/Users/pooja.bhat/Dropbox (VBC)/Paperoutline/figureOutline/paper_ZebrafishSLAMseq/other/rawfigures/figure5/data/FractionU.txt",
            sep="\t",quote = F,row.names = F)
write.table(increaseOvertime,"/Volumes/Macintosh HD/Users/pooja.bhat/Dropbox (VBC)/Paperoutline/figureOutline/paper_ZebrafishSLAMseq/other/rawfigures/figure5/data/meanFractionU.txt",
            sep="\t",quote = F,row.names = F)
...
write.table(pvals, "/Volumes/Macintosh HD/Users/pooja.bhat/Dropbox (VBC)/Paperoutline/figureOutline/paper_ZebrafishSLAMseq/other/rawfigures/figure5/data/pvals_mstable_fractionU.txt",
            sep="\t",quote = F, row.names = F)
